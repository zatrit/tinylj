...
impl State {
    ...
    /// Opens the tinylj JIT library on this state.
    ...
    pub fn pcall(
        ...
        let res: ThreadStatus = unsafe { lua_pcall(self.state, nargs, nres, err_func).into() };
    ...
    pub fn pcallx(&mut self, nargs: i32, nres: i32, err_func: i32) -> ThreadStatus {
        unsafe { lua_pcall(self.state, nargs, nres, err_func).into() }
    }
    ...
    ///
    /// ```
    /// use tinylj::{State, ThreadStatus, c_int};
    /// use tinylj::ffi::lua_State;
    ///
    ...
    pub fn to_str(&mut self, idx: c_int) -> Option<&str> {
        let ptr = unsafe { lua_tostring(self.state, idx) };
    ...
    pub fn to_int(&mut self, idx: c_int) -> Option<i32> {
        if self.is_number(idx) {
            unsafe { Some(lua_tointeger(self.state, idx) as i32) }
        } else {
            None
        }
    }
    ...
    pub fn to_long(&mut self, idx: c_int) -> Option<i64> {
        if self.is_number(idx) {
            unsafe { Some(lua_tointeger(self.state, idx) as i64) }
        } else {
            None
        }
    }
    ...
    pub fn to_bool(&mut self, idx: c_int) -> Option<bool> {
        if self.is_bool(idx) {
            unsafe { Some(lua_toboolean(self.state, idx) != 0) }
        } else {
            None
        }
    }
    ...
    pub fn to_float(&mut self, idx: c_int) -> Option<f32> {
        if self.is_number(idx) {
            unsafe { Some(lua_tonumber(self.state, idx) as f32) }
        } else {
            None
        }
    }
    ...
    pub fn to_double(&mut self, idx: c_int) -> Option<f64> {
        if self.is_number(idx) {
            unsafe { Some(lua_tonumber(self.state, idx)) }
        } else {
            None
        }
    }
    ...
    pub fn to_raw_userdata(&mut self, idx: c_int) -> Option<*mut c_void> {
        if self.is_userdata(idx) {
            unsafe { Some(lua_touserdata(self.state, idx)) }
        } else {
            None
        }
    }
    ...
    pub fn check_userdata_ex<T>(&mut self, idx: c_int, ty: &str) -> Option<*mut T> {
        unsafe {
            let udata = luaL_checkudata(self.state, idx, ptr!(ty));
    ...
    pub fn set_global(&mut self, name: &str) {
        unsafe {
            lua_setglobal(self.state, ptr!(name));
        }
    }
    ...
    pub fn set_field(&mut self, idx: i32, name: &str) {
        unsafe {
            lua_setfield(self.state, idx, ptr!(name));
        }
    }
    ...
    pub fn register_fns(&mut self, name: Option<&str>, mut fns: Vec<luaL_Reg>) {
        ...
        match name {
            Some(s) => unsafe {
                luaL_register(self.state, ptr!(s), fns.as_ptr());
            },
            None => unsafe {
    ...
    ///
    /// use tinylj::{State, LuaObject, c_int};
    ...
    ///     assert_eq!(res, tinylj::ThreadStatus::Ok);
    /// }
    /// ```
    ...
    pub fn get_global(&mut self, name: &str) {
        self.checkstack(1);
        unsafe {
            lua_getglobal(self.state, ptr!(name));
        }
    }
    ...
    pub fn get_field(&mut self, idx: i32, name: &str) {
        self.checkstack(1);
        unsafe {
            lua_getfield(self.state, idx, ptr!(name));
        }
    }
    ...
    pub fn load_file(&mut self, path: &Path) -> Result<(), (ThreadStatus, String)> {
        if path.is_file() {
            ...
            unsafe {
                let res: ThreadStatus = luaL_loadfile(self.state, ptr!(full_path.as_ref())).into();
                if res != ThreadStatus::Ok {
                    Err((res, self.to_str(-1).unwrap_or_default().to_owned()))
                } else {
                    Ok(())
    ...
    pub fn checkstack(&mut self, n: usize) -> bool {
        unsafe { lua_checkstack(self.state, n as c_int) != 0 }
    }
}
